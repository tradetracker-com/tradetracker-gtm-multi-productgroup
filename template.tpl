___INFO___

{
  "displayName": "TradeTracker.com",
  "description": "TradeTracker.com Conversion and Leads Tracking Tag. This version supports Multi ProductGroup campaigns.",
  "securityGroups": [],
  "id": "cvt_temp_public_id",
  "type": "TAG",
  "version": 1,
  "brand": {
    "thumbnail": "data:image/png;base64,",
    "displayName": "",
    "id": "brand_dummy"
  },
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "help": "Please select the type of tag that you need to implement:</br></br>  \n\n<strong>Sales Tag</strong> tracks sales events (retail orders) on your site using a single TradeTracker Product-Group ID for a standard campaign.</br></br>\n\n<strong>Leads Tag</strong> tracks leads events, which can be any visitor actions such as newsletter signups or demo requests.</br></br>\n\n<strong>Sales - Multi Product Group</strong> tag tracks sales events on your site for a multiple product group campaign where different categories can have different product-groups for tracking commission rates.</br></br>",
    "macrosInSelect": false,
    "selectItems": [
      {
        "displayValue": "Sales Tag",
        "value": "sale"
      },
      {
        "displayValue": "Leads Tag",
        "value": "lead"
      },
      {
        "displayValue": "Sales - Multi Product Group",
        "value": "multi"
      }
    ],
    "displayName": "Tag Type",
    "simpleValueType": true,
    "name": "tagType",
    "type": "SELECT"
  },
  {
    "help": "Please enter your TradeTracker.com Campaign Id, or a variable that will return this value. If you run multiple campaigns please ensure it is the correct Id.</br></br>If you're unsure of what number to use, please  check in your Advertiser Dashboard or contact your Account Manager for assistance.",
    "displayName": "Campaign Id",
    "simpleValueType": true,
    "name": "campaignID",
    "type": "TEXT"
  },
  {
    "help": "Please enter your TradeTracker.com Product Group ID, or a variable that contains this value.</br></br> If you're unsure about what number to use, please check in your Advertiser Dashboard or contact your Account Manager for assistance.</br></br> \n\n<em><strong>Multi Product Group Tag:</strong>\nIf you're using the multi product-group tag type, this is the default Product-Group that will be used if the tag cannot match basket items to a category specific Product-Group.</em>",
    "displayName": "Product Group ID",
    "simpleValueType": true,
    "name": "productID",
    "type": "TEXT"
  },
  {
    "help": "Please select the dataLayer variable for your Transaction or Order Id.</br></br>For the <strong>Sales tag</strong> this should be the same Id that you use to track sales internally, and it should be unique for every transaction.</br></br>For the <strong>Leads tag</strong> the Transaction Id can be any number you chose but also must be unique.",
    "displayName": "Transaction Id",
    "simpleValueType": true,
    "name": "orderID",
    "type": "TEXT"
  },
  {
    "help": "Please select the dataLayer variable for your Transaction or Order Id.</br></br>For the <strong>Sales tag</strong> this should be the same Id that you use to track sales internally, and it should be unique for every transaction.</br></br>For the <strong>Leads tag</strong> the Transaction Id can be any number you chose but also must be unique.",
    "enablingConditions": [
      {
        "paramName": "tagType",
        "type": "NOT_EQUALS",
        "paramValue": "leads"
      }
    ],
    "displayName": "Transaction Amount",
    "simpleValueType": true,
    "name": "orderAmount",
    "type": "TEXT"
  },
  {
    "enablingConditions": [
      {
        "paramName": "tagType",
        "type": "EQUALS",
        "paramValue": "multi"
      }
    ],
    "displayName": "Basket Items Array",
    "simpleValueType": true,
    "name": "basketItems",
    "type": "TEXT",
    "help": "Please select the dataLayer variable for the Basket Item in a sale.</br></br>\nThis should be an array of product objects, please refer to our documentation if you're unsure of which variable to use, or how to create the required variable."
  },
  {
    "enablingConditions": [
      {
        "paramName": "tagType",
        "type": "EQUALS",
        "paramValue": "multi"
      }
    ],
    "displayName": "Additional ProductGroups",
    "name": "extraProductIds",
    "simpleTableColumns": [
      {
        "defaultValue": "",
        "displayName": "ProductGroup ID",
        "name": "pid",
        "type": "TEXT",
        "isUnique": true
      },
      {
        "defaultValue": "",
        "displayName": "Category Keywords",
        "name": "categories",
        "type": "TEXT",
        "isUnique": false
      }
    ],
    "type": "SIMPLE_TABLE",
    "help": "Add additional Product Groups by entering a ProductGroup ID in the left column, and a comma-separated list of category keywords for product-categories that should be matched to that product-group. </br></br>\n\n<em><strong>Example:</strong>You have a product-group for glasses and contacts, you can enter \"glasses, contacts\" as the category keyword.</em></br></br>\n\nIt is also possible to enter regular expressions as category keywords to make matching more precise. </br></br>\n\nIf you need any help setting up the additional Product Group Ids, please refer to our documentation, or contact your Account Manager for assistance.",
    "newRowButtonText": "Add Product Group"
  }
]


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "send_pixel",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urls",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "https://*.tradetracker.net/*"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

const encodeUriComponent = require('encodeUriComponent');
const query = require('queryPermission');
const sendPixel = require('sendPixel');

function mapPid(productGroups, category) {
  for (var pid in productGroups) {
    var cArray = productGroups[pid];
    for (var i = 0, len = cArray.length; i < len; i++) {
      if (category.search(cArray[i].toLowerCase()) != -1) {
        return pid;
      }
    }
  }
  return productID;
}

function mapProducts(p) {
  return p.map(function (a) {
    return {
      'amount': a.price,
      'productID': mapPid(productGroups, a.category.join(' > ').toLowerCase())
    };
  });
}
const find = (array, func) => {
  for (let i = 0; i < array.length; i++) {
    if (func(array[i])) return array[i];
  }
  return null;
};

const domain = data.tagType === 'lead' ? 'tl.tradetracker.net' : 'ts.tradetracker.net';
const campaignID = data.hasOwnProperty('campaignID') ? '' + data.campaignID : '';
const productID = data.hasOwnProperty('productID') ? '' + data.productID : '';
const orderID = data.hasOwnProperty('orderID') ? '' + data.orderID : '';
const products = data.hasOwnProperty('basketItems') ? data.basketItems : [];
const orderAmount = data.hasOwnProperty('orderAmount') ? '' + data.orderAmount : '';
const extraProductIds = data.hasOwnProperty('extraProductIds') ? data.extraProductIds : [];
let imgUrl = '';

const productGroups = extraProductIds.reduce(function (map, obj) {
  map[obj.pid] = obj.categories.replace(' ', '').split(',');
  return map;
}, {});

const basket = mapProducts(products).reduce(function (acc, b) {
  let found = find(acc, x => x.productID === b.productID);
  if (found) {
    found.amount += b.amount;
  }
  else {
    acc.push({ productID: b.productID, amount: b.amount });
  }
  return acc;
}, []);

if (data.tagType === 'multi') {
  for (let i = 0; i < basket.length; i++) {
    let imgUrl = 'https://' + domain + '/?cid=' + encodeUriComponent(campaignID) + '&pid=' + encodeUriComponent(basket[i].productID) + '&tid=' + encodeUriComponent(orderID) + '&data=&descrMerchant=&descrAffiliate=&tam=' + encodeUriComponent(basket[i].amount.toString()) + '&event=sales&qty=1&currency=EUR&vc=';
    sendPixel(imgUrl, null, data.gtmOnFailure);
  }
  data.gtmOnSuccess();
} else {
  imgUrl += 'https://' + domain + '/?cid=' + encodeUriComponent(campaignID) + '&pid=' + encodeUriComponent(productID) + '&tid=' + encodeUriComponent(orderID) + '&data=&descrMerchant=&descrAffiliate=';
  if (data.tagType === 'sale') {
    imgUrl += '&tam=' + encodeUriComponent(orderAmount) + '&event=sales&qty=1&currency=EUR&vc=';
  } else {
    imgUrl += '&event=lead';
  }
  if (query('send_pixel', imgUrl)) {
    sendPixel(imgUrl, data.gtmOnSuccess, data.gtmOnFailure);
  }
}


___NOTES___

Created on 29/08/2019, 16:10:24
